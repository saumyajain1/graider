...
class DataEditorState_HP(rx.State):
    ...
    def get_clicked_data(self, pos) -> QuestionPart:
        col, row = pos
        ...
        if 0 <= row < len(self.data):
            ...
            self.refresh_table()
            return current_question_part
        return None

    def delete_question(self):
         curr = self.get_clicked_data
         if curr != None:
            api.question_parts.remove(curr)
    ...
    def get_edited_data(self, pos, new_value):
        ...
        if 0 <= row < len(self.data):
            ...
            api.question_parts.remove(current_question_part)
            print_question_parts

            api.question_parts.append(updated_question_part)
            print_question_parts
            # # Delete the old QuestionPart and insert the new one in the same position
            # del api.question_parts[row]  # Remove the old QuestionPart
            # api.question_parts.insert(row, updated_question_part)  # Insert the updated QuestionPart at the same index
        
            self.refresh_table()

        # Optionally print the updated question parts for debugging
        print_question_parts()
...
class QuestionState(rx.State):
    ...
    def handle_submit(self, form_data: dict):
        ...
        # # Debugging: print question parts
        # print_question_parts()
